...
- **Next.js API Routes over standalone backend:** GoalGetter implements its backend directly within the Next.js /api directory rather than maintaining a decoupled backend service. **Trade-off:** A dedicated backend service offers more granular scaling and language flexibility. However, Next.js allows for a single-repo architecture where the frontend and backend are tightly coupled and deploy together seamlessly. This reduces deployment complexity and increases feature velocity, making it a good fit for a solo-maintained project.
...
GRANT ALL PRIVILEGES ON DATABASE goalgetter_db TO goalgetter_user;
ALTER DATABASE goalgetter_db OWNER TO goalgetter_user;
\q
psql -U goalgetter_user -d goalgetter_db -f schema.sql
```
