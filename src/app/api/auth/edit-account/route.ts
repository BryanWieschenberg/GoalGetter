...
export const PATCH = withAuth(async (req, userId) => {
    ...
    try {
        ...
        if (isLocal) {
            ...
            if (editType === "email") {
                return NextResponse.json(
                    { error: "OAuth accounts cannot change email" },
                    { status: 400 },
                );
            }
        }
        ...
        if (editType === "handle" && newValue === user.handle) {
            return NextResponse.json(
                { error: "New handle cannot be the same as the current one." },
                { status: 400 },
            );
        }
        if (editType === "email" && newValue === user.email) {
            return NextResponse.json(
                { error: "New email cannot be the same as the current one." },
                { status: 400 },
            );
        }
        ...
        if (editType === "username") {
            ...
            const userRes = await client.query("SELECT id, username FROM users WHERE email=$1", [
                user.email,
            ]);
